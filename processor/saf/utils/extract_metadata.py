...
class ExtractMetadata:
    ...
    def __process_iterable_map(self, parent_element, elements, element_map):
        ...
        :param element_map: the dictionary for mets to dspace mapping.
        """
        if elements is not None:
            ...
            # Since we may never face this situation, it's a violation of the
    ...
    def extract_metadata(self, record):
        ...
        mods_elements = self.tag_names.mets_fields
        ...
        mods_keys = sorted(mods_elements.keys())

        for key in mods_keys:
            # The mods_elements value for attributes is always an array.
            if mods_elements[key].get('attr_val') is not None:
                # For each value in the attribute array, create a separate saf element.
                for val in mods_elements[key].get('attr_val'):
                    if val is not None:
                        # Look up the element by attribute in the mets xml and add to
                        # saf xml if elements are found.
                        temp_elements = mods_elements[key].copy()
                        temp_elements['attr_val'] = [val]
                        elements = self.__get_mods_element(temp_elements, section)
                        if len(elements) > 0:
                ...
                elements = self.__get_mods_element(mods_elements[key], section)
                if elements is not None:
